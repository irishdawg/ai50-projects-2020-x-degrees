...
def shortest_path(source, target):
    ...
    # Load movies that the source starred in to initialize the frontier
...
#            return solution
        ...
        for state in neighbors_for_person(node.state[1]):  
            ...
            if not frontier.contains_state(state) and state not in explored:
                ...
                # can check here for a hit 
                if state[1] == target:
                    print("Total nodes explored {}".format(nodes_explored))
                    solution = [state]
                    while node.parent is not None:
                        solution.append(node.state)
                        node = node.parent
